        ...
        for cur_epoch in range(FLAGS.num_epochs):
            ...
            for cur_batch in range(num_batches_per_epoch):
                ...
                if step % FLAGS.save_steps == 1:
                    ...
                    #Write graph
                    #tf.train.write_graph(sess.graph_def, FLAGS.checkpoint_dir, 'graph.pbtxt', as_text=False)
...
def infer(img_path, mode='infer'):
    ...
    with tf.Session(config=config) as sess:
        ...
        for curr_step in range(int(total_steps)):
            ...
            dense_decoded_code, prob = sess.run([model.dense_decoded, model.log_prob], feed)
            for cnt, item in enumerate(dense_decoded_code):
                expression = ''
                ...
                decoded_expression.append(expression+'\t'+str(round(math.exp(prob[cnt]), 4)))
