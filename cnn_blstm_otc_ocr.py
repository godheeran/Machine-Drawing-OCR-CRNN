...
class LSTMOCR(object):
    ...
    def _build_model(self):
        ...
        with tf.variable_scope('cnn'):
            ...
            for i in range(FLAGS.cnn_count):
                with tf.variable_scope('unit-%d' % (i + 1)):
                    ...
                    # else:
                    #     x = self._max_pool_m(x, 2, strides[1])
        ...
        with tf.variable_scope('lstm'):
            ...
            # tf.nn.rnn_cell.RNNCell, tf.nn.rnn_cell.GRUCell, LSTMCell
            ...
            if self.mode == 'infer':
                cell1 = tf.nn.rnn_cell.DropoutWrapper(cell=cell1, output_keep_prob=1.0)
            if self.mode == 'train':
                cell1 = tf.nn.rnn_cell.DropoutWrapper(cell=cell, output_keep_prob=FLAGS.output_keep_prob)
            ...
            self.logits = tf.add(tf.matmul(outputs, W), b, name='logits')
    ...
    def _build_train_op(self):
        ...
        self.dense_decoded = tf.sparse_tensor_to_dense(self.decoded[0], default_value=-1, name='dense_decode')
    ...
    def _max_pool_m(self, x, ksize, strides):
            return tf.nn.max_pool(x,
                                ksize=[1, ksize, 1, 1],
